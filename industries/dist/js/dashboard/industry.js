...
// ── VIEW OPPORTUNITY MODAL (INDEX & PROBLEM NUMBER BASED) ──
window.viewOpportunity = function(key) {
  const cleanKey = String(key || '').trim();
  const allList = (window._allExploreOpportunities && window._allExploreOpportunities.length > 0)
    ? window._allExploreOpportunities
    : (_allExploreOpportunities || []);

  // 1. Index-based & Problem Number match
  const item = allList.find(o => 
    String(o.code) === cleanKey || 
    String(o.code || '').replace('#', '') === cleanKey.replace('#', '') ||
    String(o._id) === cleanKey ||
    String(o.challengeId) === cleanKey
  ) || (_rawChallengesList || []).find(c => 
    String(c.challengeId) === cleanKey || 
    String(c.challengeId || '').replace('#', '') === cleanKey.replace('#', '') ||
    String(c._id) === cleanKey
  );

  if (!item) {
    console.warn('Opportunity not found for problem key:', key);
    return;
  }

  // Populate #modalChallengeDetails with exact matching card data
  const codeEl = document.getElementById('detailModalChallengeId');
  if (codeEl) codeEl.textContent = item.code || item.challengeId || '#JH-2026';

  const catEl = document.getElementById('detailModalCategory');
  if (catEl) catEl.textContent = (Array.isArray(item.domains) ? item.domains[0] : item.category) || 'Civic Infrastructure';

  const distEl = document.getElementById('detailModalDistrict');
  if (distEl) distEl.textContent = `📍 ${item.district || 'Jharkhand'}, ${item.state || 'Jharkhand'}`;

  const prioEl = document.getElementById('detailModalPriorityBadge');
  if (prioEl) {
    const isUrgent = item.priority && (item.priority.includes('High') || item.priority.includes('Urgent'));
    prioEl.className = 'badge';
    prioEl.style.cssText = `background: ${isUrgent ? '#fef2f2' : '#fffbeb'}; color: ${isUrgent ? '#b91c1c' : '#d97706'}; font-weight: 800; border: 1px solid currentColor;`;
    prioEl.textContent = item.priority || 'High Priority';
  }

  const statusEl = document.getElementById('detailModalStatusBadge');
  if (statusEl) {
    statusEl.className = 'badge';
    statusEl.style.cssText = `background: ${item.stageColor || '#f5f3ff'}; color: ${item.stageTextColor || '#7c3aed'}; font-weight: 800; border: 1px solid currentColor;`;
    statusEl.textContent = item.stage || 'Solution Blueprinting';
  }
  ...
  if (subEl) {
    subEl.textContent = `${item.university || 'State Innovation Partner'} · PI: ${item.lead || 'Faculty Lead'} · 📅 Expected: ${item.expectedDate || 'Q3-Q4 2026'}`;
  }

  const bodyEl = document.getElementById('challengeDetailsModalBody');
  if (bodyEl) {
    bodyEl.innerHTML = `
      <div style="display: flex; gap: 22px; flex-wrap: wrap; margin-bottom: 20px; align-items: flex-start;">
        <div style="width: 240px; height: 160px; border-radius: 12px; overflow: hidden; border: 1.5px solid #cbd5e1; flex-shrink: 0; background: #f8fafc; box-shadow: 0 4px 14px rgba(0,0,0,0.06);">
          <img src="${item.thumbnail || '/images/campus-iit.jpg'}" alt="${item.title}" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.onerror=null;this.src='/images/agri-monitoring.jpg';" />
        </div>
        <div style="flex: 1; min-width: 260px;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            <span style="font-size: 12.5px; color: #64748b;">Problem Identifier: <strong style="color: #002D62; font-size: 14px;">${item.code || item.challengeId}</strong></span>
            <span style="background: #dcfce7; color: #15803d; font-size: 12px; font-weight: 850; padding: 3px 10px; border-radius: 99px; border: 1px solid #86efac;">
              🟢 ${item.aiMatch || 96}% Match Score
            </span>
          </div>
          <div style="font-size: 13.5px; color: #1e293b; line-height: 1.6; margin-bottom: 14px;">
            ${item.description || 'Community challenge verified by state administration for multi-stakeholder technical blueprinting and CSR industry support.'}
          </div>
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 10px; background: #f8fafc; padding: 12px 14px; border-radius: 10px; border: 1px solid #e2e8f0; font-size: 12px;">
            <div><strong>Academic Partner:</strong><br/><span style="color: #0f172a; font-weight: 700;">${item.university}</span></div>
            <div><strong>Taskforce Lead:</strong><br/><span style="color: #0f172a; font-weight: 700;">${item.lead}</span></div>
            <div><strong>Estimated CSR Support:</strong><br/><span style="color: #15803d; font-weight: 850;">${item.estimatedBudget}</span></div>
            <div><strong>Location:</strong><br/><span style="color: #0f172a; font-weight: 700;">${item.district}, ${item.state}</span></div>
          </div>
        </div>
      </div>

      <div style="background: #eff6ff; border: 1.5px solid #bfdbfe; border-radius: 12px; padding: 16px; margin-bottom: 16px;">
        <div style="font-size: 13px; font-weight: 850; color: #1e40af; margin-bottom: 8px;">Required Industry Support Domains:</div>
        <div style="display: flex; gap: 8px; flex-wrap: wrap;">
          ${(item.requiredSupport || ['Funding', 'Equipment', 'Technical Mentor']).map(s => `<span class="badge" style="background: #ffffff; color: #1d4ed8; border: 1px solid #93c5fd; font-weight: 750; padding: 4px 10px; border-radius: 6px;">✓ ${s}</span>`).join('')}
        </div>
      </div>
    `;
  }

  const actionContainer = document.getElementById('detailModalActionBtnContainer');
  if (actionContainer) {
    actionContainer.innerHTML = `
      <button class="btn btn-primary" onclick="window.expressInterest('${item.code || item._id}')" style="background: #002D62; font-weight: 800; padding: 9px 18px; border-radius: 10px; box-shadow: 0 4px 12px rgba(0,45,98,0.2);">
        Express Interest →
      </button>
    `;
  }

  const m = document.getElementById('modalChallengeDetails');
  if (m) {
    m.classList.add('open', 'active');
    m.style.display = 'flex';
  }
...
// ── EXPRESS INTEREST (NOTIFIES ADMIN WITH PROBLEM NUMBER, TITLE & INDUSTRY NAME) ──
window.expressInterest = async function(key) {
  const cleanKey = String(key || '').trim();
  const allList = (window._allExploreOpportunities && window._allExploreOpportunities.length > 0)
    ? window._allExploreOpportunities
    : (_allExploreOpportunities || []);
  ...
  const probNum = item.code || item.challengeId || '#JH-2026-CHALLENGE';
  const probTitle = item.title || 'Community Civic Challenge';

  // 1. Send live notification to Admin via Backend
  try {
    const token = sessionStorage.getItem('token') || sessionStorage.getItem('is_token') || '';
    const headers = { 'Content-Type': 'application/json' };
    if (token) headers['Authorization'] = `Bearer ${token}`;

    const res = await fetch('/api/industry/express-interest', {
      method: 'POST',
      headers,
      body: JSON.stringify({
        problemId: item._id,
    ...
    console.log('Admin notified of interest:', resData);
  } catch (err) {
    console.warn('Failed to send interest notification to admin:', err);
  }

  // 2. Immediate Toast Confirmation
  toastSuccess(
    `Admin notified! Interest expressed for ${probNum}: "${probTitle}" from ${orgName}`,
    'Interest Expressed to Admin'
  );

  // 3. Pre-fill and open partnerModal so industry can customize pledge or chat
  const propTitleInput = document.getElementById('propChallengeTitle');
  if (propTitleInput) propTitleInput.value = probTitle;

  const partnerHeader = document.getElementById('partnerProjectTitle');
  if (partnerHeader) partnerHeader.textContent = probTitle;

  const collabCodeBadge = document.getElementById('collabModalChallengeId');
  if (collabCodeBadge) collabCodeBadge.textContent = probNum;

  const collabUnivBadge = document.getElementById('collabModalUniversity');
  if (collabUnivBadge) collabUnivBadge.textContent = item.university || 'University Partner';

  window._currentExpressedInterest = {
    problemId: item._id,
    problemNumber: probNum,
    problemTitle: probTitle,
    industryName: orgName
  ...
  if (partnerModal) {
    partnerModal.classList.add('open', 'active');
    partnerModal.style.display = 'flex';
  }
};

// ── SUBMIT DETAILED INTEREST PLEDGE FORM ──
window.submitPartnerInterest = async function() {
  const current = window._currentExpressedInterest || {};
  const orgName = currentUser?.organization || currentUser?.companyName || 'Tata Steel Foundation';
  const probNum = current.problemNumber || '#JH-2026';
  const probTitle = current.problemTitle || 'Challenge';

  const fundingVal = document.getElementById('collabFundingAmount')?.value || '1200000';
  const messageVal = document.getElementById('partnerMessage')?.value || 'We express strong CSR and technical mentoring interest for this challenge.';
  ...
  try {
    ...
    await fetch('/api/industry/express-interest', {
      method: 'POST',
      headers,
      body: JSON.stringify({
  ...
  toastSuccess(
    `Detailed collaboration pledge submitted to Admin for ${probNum}: "${probTitle}"!`,
    'Pledge Registered'
  );
  window.closeModal && window.closeModal('partnerModal');
};
