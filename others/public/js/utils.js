...
// ── Auth Token Management (Multi-Role & Tab-Isolated) ──
var Auth = {
  getRoleFromUrl: () => {
    try {
      ...
      if (path.includes('/industr')) return 'industry';
      if (path.includes('/citizen')) return 'citizen';
    } catch(e) {}
    return null;
  },

  getToken: (preferredRole = null) => {
    const currentRole = preferredRole || Auth.getRoleFromUrl();

    // 1. Tab-isolated session storage (highest priority)
    try {
      const tabToken = sessionStorage.getItem('token') || sessionStorage.getItem('is_token');
      const tabRole = sessionStorage.getItem('user_role') || sessionStorage.getItem('role');
      if (tabToken) {
        if (!currentRole || !tabRole) return tabToken;
        const normTab = (tabRole === 'admin') ? 'admin' :
                        (tabRole === 'university_rep' || tabRole === 'university') ? 'university' :
                        (tabRole === 'industry_rep' || tabRole === 'industry') ? 'industry' : 'citizen';
        if (normTab === currentRole) return tabToken;
      }
    } catch(e) {}

    // 2. Any session token in this tab
    try {
      const anyTabToken = sessionStorage.getItem('token') || sessionStorage.getItem('is_token');
      if (anyTabToken) return anyTabToken;
    } catch(e) {}

    // Auth must remain tab-scoped. Shared localStorage would make a new tab
    // silently inherit another tab's role and profile.
    return '';
  },

  getUser: (preferredRole = null) => {
    try {
      const currentRole = preferredRole || Auth.getRoleFromUrl();

      // 1. Tab-isolated session user
      const tabRaw = sessionStorage.getItem('user') || sessionStorage.getItem('is_user');
      if (tabRaw) {
        const tabUser = JSON.parse(tabRaw);
        if (!currentRole) return tabUser;
        const normTab = (tabUser.role === 'admin') ? 'admin' :
                        (tabUser.role === 'university_rep' || tabUser.role === 'university') ? 'university' :
                        (tabUser.role === 'industry_rep' || tabUser.role === 'industry') ? 'industry' : 'citizen';
        if (normTab === currentRole) return tabUser;
      }

      // 2. Tab user fallback (only if matching role or no role required)
      if (tabRaw) {
        const tabUser = JSON.parse(tabRaw);
        if (!currentRole) return tabUser;
        const normTab = (tabUser.role === 'admin') ? 'admin' :
  ...
  setAuth: (token, user, roleOverride = null) => {
    const rawRole = roleOverride || (user && user.role) || 'citizen';
    const normRole = (rawRole === 'admin') ? 'admin' :
                     (rawRole === 'university_rep' || rawRole === 'university') ? 'university' :
                     (rawRole === 'industry_rep' || rawRole === 'industry') ? 'industry' : 'citizen';

    // Tab-isolated storage (won't touch other tabs!)
    try {
      sessionStorage.setItem('token', token);
      sessionStorage.setItem('is_token', token);
      sessionStorage.setItem('user', JSON.stringify(user));
      sessionStorage.setItem('is_user', JSON.stringify(user));
      sessionStorage.setItem('user_role', user.role || normRole);
      sessionStorage.setItem('role', user.role || normRole);
    } catch(e) {}

  },

  clearAuth: (roleOnly = null) => {
    try { sessionStorage.clear(); } catch(e) {}
    const roleToClear = roleOnly || Auth.getRoleFromUrl();
  },

  isLoggedIn: (preferredRole = null) => !!Auth.getToken(preferredRole),
  redirectToDashboard: (role) => {
    ...
    window.location.replace(routes[role] || '/citizen');
  },
  requireAuth: () => {
    if (!Auth.isLoggedIn()) {
