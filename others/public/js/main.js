    ...
    if (user) {
      ...
      if (actions) {
        actions.innerHTML = `
          ...
          <a href="/login.html" class="btn-nav-login" title="Sign in as another role or open in new tab" style="font-weight:700;color:#002D62;background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:8px;padding:6px 12px;text-decoration:none;display:inline-flex;align-items:center;gap:5px;font-size:13px;transition:all 0.2s ease;">
...
window.instantDemoLogin = async (email, password, openInNewTab = null) => {
  try {
    const btn = window.event ? window.event.currentTarget : null;
    const shouldOpenNewTab = (openInNewTab === true) || (document.getElementById('chkOpenInNewTab')?.checked === true);
    if (btn) {
      btn.style.opacity = '0.6';
      btn.textContent = '⏳ Launching portal...';
    }
    ...
    if (json.success && json.token && json.user) {
      const role = json.user.role || 'citizen';
      const normRole = (role === 'admin') ? 'admin' :
                       (role === 'university_rep' || role === 'university') ? 'university' :
                       (role === 'industry_rep' || role === 'industry') ? 'industry' : 'citizen';

      if (typeof Auth !== 'undefined' && Auth.setAuth) {
        Auth.setAuth(json.token, json.user, normRole);
      } else {
        try {
          sessionStorage.setItem('token', json.token);
          sessionStorage.setItem('is_token', json.token);
          sessionStorage.setItem('user', JSON.stringify(json.user));
          sessionStorage.setItem('is_user', JSON.stringify(json.user));
          sessionStorage.setItem('user_role', json.user.role);
          localStorage.setItem('token_' + normRole, json.token);
          localStorage.setItem('user_' + normRole, JSON.stringify(json.user));
          localStorage.setItem('role_' + normRole, json.user.role);
          localStorage.setItem('token', json.token);
          localStorage.setItem('is_token', json.token);
          localStorage.setItem('user', JSON.stringify(json.user));
          localStorage.setItem('is_user', JSON.stringify(json.user));
        } catch(e) {}
      }
      ...
      const dest = routes[json.user.role] || '/citizen';
      if (shouldOpenNewTab) {
      ...
      alert('Login notice: ' + (json.message || 'Please check login credentials'));
...
// ── Instant 1-Click Demo Login Handler for Landing Page ──
window.instantDemoLogin = async (email, password) => {
  try {
