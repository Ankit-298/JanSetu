...
exports.getAdminAnalytics = async (req, res, next) => {
  try {
    ...
    const [
      ...
      User.countDocuments({ role: 'industry_rep', industryPartnerId: { $ne: null } }),
      ...
      Challenge.countDocuments({ status: { $in: ['assigned', 'in_progress', 'testing', 'Assigned'] } }),
...
exports.getIndustryPartners = async (req, res, next) => {
  try {
    ...
    const [partners, total] = await Promise.all([
      IndustryProfile.find({ _id: { $in: registeredIndustryIds }, isActive: { $ne: false } }).sort('-createdAt').skip((parseInt(page) - 1) * parseInt(limit)).limit(parseInt(limit)),
      IndustryProfile.countDocuments({ _id: { $in: registeredIndustryIds }, isActive: { $ne: false } })
    ]);
